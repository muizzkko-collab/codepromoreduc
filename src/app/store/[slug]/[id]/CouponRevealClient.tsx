...
export function CouponRevealClient({
  ...
  storeName, storeSlug, logoUrl, bannerUrl,
  ...
  const handleCopyAndGo = useCallback(() => {
    const openAffiliate = () => window.open(affiliateUrl, '_blank', 'noopener,noreferrer')
    if (hasCode && couponCode) {
  ...
  }, [couponCode, hasCode, affiliateUrl])

  const from = `/store/${storeSlug}/`
          ...
          <div style={{ padding: '32px 36px 24px', textAlign: 'center', borderBottom: '1px solid rgba(255,255,255,.06)' }}>
            ...
            {discountValue && (
              <div style={{
                ...
                <span style={{ fontSize: 18, fontWeight: 900, color: '#c4b5fd', letterSpacing: '-.01em' }}>{discountValue}</span>
            ...
            {hasCode ? (
              <>
                ...
                <div
                  ...
                  role="button"
                  tabIndex={0}
                  onKeyDown={e => e.key === 'Enter' && handleCopyAndGo()}
                  style={{
                ...
                >
                  ...
                  {copied ? <CheckCheck size={24} color="#22c55e" /> : <Copy size={24} color="rgba(165,180,252,.7)" />}
                </div>
                ...
                <button
                  ...
                  onMouseEnter={e => {
                    if (!copied) {
                      e.currentTarget.style.background = 'linear-gradient(135deg, rgba(99,102,241,.75) 0%, rgba(139,92,246,.65) 50%, rgba(168,85,247,.6) 100%)'
                      e.currentTarget.style.boxShadow  = '0 12px 40px rgba(99,102,241,.6), inset 0 1px 0 rgba(255,255,255,.3), inset 0 -1px 0 rgba(0,0,0,.2)'
                  ...
                  onMouseLeave={e => {
                    ...
                    if (!copied) {
                      e.currentTarget.style.background = 'linear-gradient(135deg, rgba(99,102,241,.55) 0%, rgba(139,92,246,.45) 50%, rgba(168,85,247,.4) 100%)'
                      e.currentTarget.style.boxShadow  = '0 8px 32px rgba(99,102,241,.45), inset 0 1px 0 rgba(255,255,255,.25), inset 0 -1px 0 rgba(0,0,0,.2)'
                    }
                  }}
                >
